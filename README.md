...
「設定」タブから、[音ゲーツール置き場(reiwa.f5.si)](https://reiwa.f5.si/) が公開している `chunithm_record.json` を使って、ChuniSupport側にまだ定数が無い曲の「定数不明」を補完することもできます(既存の値は上書きしません)。
...
譜面定数の補完機能は [音ゲーツール置き場(reiwa.f5.si)](https://reiwa.f5.si/) の `chunithm_record.json` を利用しています。[利用上の注意](https://reiwa.f5.si/api.html) に従い、非営利目的の範囲でご利用ください。取得したデータはブラウザに12時間キャッシュされ、過度な連続取得を避ける設計にしています。
